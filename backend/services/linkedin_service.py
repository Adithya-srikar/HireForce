...
async def fetch_linkedin_profile(url: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            first = items[0]
            # Actor returns {"error": "..."} when Apify quota is exhausted or scraping fails
            if isinstance(first, dict) and first.get("error"):
                err_msg = str(first["error"]).strip()
                if "hard limit" in err_msg or "free user" in err_msg or "paid" in err_msg:
                    err_msg = "Apify free tier limit reached. Upgrade your Apify plan to scrape LinkedIn profiles."
                return {"error": err_msg, "username": username, "profile_url": profile_url}

            return _parse_profile(first, username, profile_url)
